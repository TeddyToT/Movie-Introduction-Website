...
const { data: popularMovies, isLoading: popularMoviesLoading, error: popularMoviesError } = usePopularMovies(1);
  const { data: topRatedMovies, isLoading: topRatedMoviesLoading, error: topRatedMoviesError } = useTopRatedMovies(1);

  const { data: popularTVs, isLoading: popularTVsLoading, error: popularTVsError } = usePopularTVs(1);
  const { data: topRatedTVs, isLoading: topRatedTVsLoading, error: topRatedTVsError } = useTopRatedTVs(1);



  if (popularMoviesLoading || topRatedMoviesLoading || popularTVsLoading || topRatedTVsLoading) {
    return <p className="text-white">Loading...</p>;
  }

  if (popularMoviesError || topRatedMoviesError || popularTVsError || topRatedTVsError) {
